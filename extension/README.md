...
|---|---|
| 0.20.0 | **Imports a published novel back into the library.** The site answers a whole novel, text included, in one request once `pages=true` is on the query — so **Check this novel** reads it and reports what would come in, and **Import it** sends exactly what was reported. Two presses on purpose: a hundred and fifty chapters arriving under the wrong name or in the wrong order is tedious to undo, so what will happen is printed before anything is written. The check names what could not be kept as Markdown, which chapters carry text Markdown would read back differently, and which ones the numbering resolver will refuse to number — the commonest cause of that last one being length, since anything under 200 non-whitespace characters with no Hangul is classed as an extra, and imported English always is. Reading happens in the page's own context with the page's own token, because hirayatales.com is deliberately absent from `host_permissions`; writing into Night Reader happens in the service worker, for the CORS reason. Also **Send novel list**, which pushes the studio's own series list (`/app/series` with no `uid` returns all of them) so the app's Import page has something to offer — done automatically once per session as well, so nobody has to remember it. |
